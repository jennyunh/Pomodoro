/* IDEAS: 
-look into https://developer.mozilla.org/en-US/docs/Web/API/Page_Visibility_API to prevent animation getting skewed when on diff tab or window


debug the app

start volume from 50%;
*/
...
class Pomodoro extends React.Component {
  ...
  componentDidMount() {
    /* when session timer is going (not on break), start the countdown */
    ...
    /* when on break, set the break time, then start countdown */
    else if ((this.state.break === true) && ((this.state.play === true))) {
      this.initiateCountDown = setInterval(this.countDown, 1000);
  ...
  /*------------------------------------------------- */
  playClicked() {
    this.setState({ play: !this.state.play }, () => { this.componentWillUnmount() });
  }
  ...
  handlePlay() {
    if (this.state.play === true) {
      ...
      //When playing, disable the settings options
    ...
    if (document.visibilityState === 'hidden' || document.visibilityState === 'visible') {
      ...
      and the component will be remounted to initiate the break = true route*/
        ...
        this.setState({
          ...
          status: "Time to Rest.",
          sessionSec: '00',
          sessionMin: this.state.breakSet
        },
          () => {
  ...
  handleBreakDec() {
    if (parseInt(this.state.breakSet, 10) > 1) { this.setState({ breakSet: (parseInt(this.state.breakSet, 10) - 1).toString() }) }
  }
...
<path id="ClockHand" d="M132.733 185.222L135.539 178.743L170.693 198.408L132.733 185.222Z" fill="black">
        ...
        <div id="session" className="animate__animated animate__jackInTheBox">
          ...
          {/* PLAY/PAUSE BUTTON */}
          <button id="playBtn" type="button" onClick={this.playClicked} >
